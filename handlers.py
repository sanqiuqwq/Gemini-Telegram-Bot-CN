...
async def start(message: Message, bot: TeleBot) -> None:
    try:
        await bot.reply_to(message , escape("欢迎，你可以问我任何问题。 \n例如: `Telegram是什么app？`"), parse_mode="MarkdownV2")
    except IndexError:
...
async def gemini_stream_handler(message: Message, bot: TeleBot) -> None:
    try:
        ...
        await bot.reply_to(message, escape("请在使用 /gemini 前添加您想说的内容。. \n例如: `Telegram是什么app？"), parse_mode="MarkdownV2")
        return
...
async def gemini_pro_stream_handler(message: Message, bot: TeleBot) -> None:
    try:
        ...
        await bot.reply_to(message, escape("请在使用 /gemini_pro 前添加您想说的内容。 \n例如: `Telegram是什么app？"), parse_mode="MarkdownV2")
        return
...
async def switch(message: Message, bot: TeleBot) -> None:
    ...
    if default_model_dict[str(message.from_user.id)] == True:
        ...
        await bot.reply_to( message , "现在你正在使用 "+model_1)
...
async def gemini_edit_handler(message: Message, bot: TeleBot) -> None:
    if not message.photo:
        await bot.reply_to(message, "请发送一张图片")
        return
    s = message.caption or ""
    try:
        m = s.strip().split(maxsplit=1)[1].strip() if len(s.strip().split(maxsplit=1)) > 1 else ""
...
async def draw_handler(message: Message, bot: TeleBot) -> None:
    try:
        ...
        await bot.reply_to(message, escape("请在发送 /draw 前添加您想要绘制的内容。 \n例如: `/draw 画一只猫`"), parse_mode="MarkdownV2")
        return
    
    # reply to the message first, then delete the "drawing..." message
    drawing_msg = await bot.reply_to(message, "绘画中...")
    try:
